...
package org.apache.samza.sql.bench.job
...
object BenchJobRunner extends Logging {
  def main(args: Array[String]) {
    val cmdline = new CommandLine
    val options = cmdline.parser.parse(args: _*)
    val config = cmdline.loadConfig(options)
    val t = if (config.get("bench.job.exectime") == null)  5 else  config.get("bench.job.exectime").toInt
    new BenchJobRunner(config, t).run
  }
}
...
/**
  ...
  */
class BenchJobRunner(config: Config, execTime: Int) extends Logging with Runnable {
  ...
  def run() {
    ...
    Await.ready(f2, (execTime * 60) seconds)
